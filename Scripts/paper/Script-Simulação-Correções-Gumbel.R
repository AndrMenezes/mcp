...
foreach(i=1:length(sd)) %do% {
  dados_xy <- rgumbel(max(ni)*2*B,location=mu,scale=sd[i])
  foreach(j=1:length(mu.a)) %do% {
    dados <- c(dados_xy,rgumbel(max(ni)*B,location=mu.a[j],scale=sd[i]))
...
# Salvando valores-p ------------------------------------------------------
setwd("C:/Users/User/Dropbox/UEM/Projeto-Simulação/Dados")

saveRDS(x6, "valorp-gumbel.rds")
...
levels(dados1$n)<-ni
levels(dados1$sd)<-sd
write.table(dados1,file="poder-gumbel.csv",sep = ";",row.names = F)
...
write.table(dados2,file="errotipoI-gumbel.csv",sep = ";",row.names = F)
